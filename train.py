...
from mindspore.train.callback import ModelCheckpoint, CheckpointConfig, TimeMonitor, SummaryCollector
...
parser.add_argument("--pre_trained", type=str, default="weights/resnet50.ckpt", help="Pretrain file path.")
...
context.set_context(mode=context.PYNATIVE_MODE, device_target="Ascend", device_id=args_opt.device_id,max_call_depth=10000)
...
if __name__ == '__main__':
    ...
    if not args_opt.only_create_dataset:
        ...
        model = Model(net, optimizer=opt, amp_level='auto')
        print("============== Starting Training ==============")
        time_cb = TimeMonitor(data_size=num_steps)
        loss_cb = LossMonitor()
        # cb = [Monitor(lr_init=lr.asnumpy(), step_num=dataset_size)]
